...
const AddExpense = ({ groupId, groupName, close,refreshData, ...props }) => {
    ...
    const AddNewExpense = (formData: any) => {
        ...
        const response = post(API_URLS.addNewExpense(groupId),payloadData,getAuthorizationHeaders())
...
return (
    ...
    >
        ...
        <div className=" bg-zinc-300/25 border rounded-2xl p-4 text-stone-700 mt-5">
            ...
            <div className="mt-2">
                <CustomInput modifyclass="placeholder:font-normal font-semibold" keyname={"paid_for"} label='Paid For*' placeholder="What did you spent on?" errors={errors} register={register} validations={{}} />
        ...
        <div className="fixed  bottom-0 left-0 right-0 py-5 mx-3 bg-white  justify-between grid grid-cols-2 gap-4  ">
            <Button varaiant={'ghost'} text={'Save & add another'}  />
            <Button varaiant={'primary'} text={'Save'} onClick={handleSubmit(AddNewExpense)} />
        </div>
